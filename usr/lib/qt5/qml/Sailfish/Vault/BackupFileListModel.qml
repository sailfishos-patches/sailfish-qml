...
ListModel {
    ...
    function _loadFromFilePath(dirPath) {
        _prepareToLoad()
        if (dirPath.length > 0) {
            var files = _backupUtils.sortedBackupFileInfo(dirPath, BackupUtils.TarArchive, false)
            if (files.length == 0) {
                files = _backupUtils.sortedBackupFileInfo(dirPath, BackupUtils.TarArchive, true)
            }
        ...
        onRequestedListing: {
            if (root.backupSource === accountId) {
                ...
                if (listing.length > 0) {
                    ...
                    var fileInfoList = _backupUtils.sortedBackupFileInfo(fileNames, BackupUtils.TarGzipArchive, false)
                    if (fileInfoList.length == 0) {
                        fileInfoList = _backupUtils.sortedBackupFileInfo(fileNames, BackupUtils.TarGzipArchive, true)
                    }
